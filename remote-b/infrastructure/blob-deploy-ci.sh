...
#
# Example:
#     infrastructure/fe-deploy-ci.sh
...
group_env="rg-remoteb"

endpoint="https://stremoteb.z6.web.core.windows.net/"
...
enabled=$(
    az_cli_container az storage blob service-properties show \
        --auth-mode login \
        --account-name "stremoteb" \
        --query 'staticWebsite.enabled' \
    | tr -d '\r\n'
)
if [ "$enabled" != 'true' ]
then
    echo "ABORTING: az storage for 'stremoteb' is not a static website"
    exit 1
fi
